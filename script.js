...
function setWallet(wallet) {
    ...
    document.querySelectorAll('.wallet-btn').forEach(btn => {
        btn.style.borderColor = btn.dataset.wallet === wallet ? 'var(--accent)' : 'var(--border)';
        btn.style.color = btn.dataset.wallet === wallet ? 'var(--accent)' : 'var(--dim)';
    });
}
        ...
        if (note.type === 'LINK') {
            ...
            card.innerHTML = `
                ...
                <div style="
                    width: 100%;
                    height: 80px;
                    background: ${imageUrl ? `url('${imageUrl}')` : 'var(--glass)'};
                ...
                ">${imageUrl ? '' : '🔗'}</div>
                <div class="title-row" style="color: var(--accent);">${title.toUpperCase()}</div>
                <div class="content-preview" style="font-size: 10px;">${description}</div>
                <div style="font-size: 9px; color: var(--accent); margin-top: 8px; opacity: 0.6;">↗ ${domain}</div>
            `;
            ...
            card.innerHTML = `
                ${isPinned ? `<div class="pin-indicator" onclick="event.stopPropagation(); togglePinFromCard('${note.id}')"><i class="fas fa-thumbtack"></i></div>` : ''}
                <div class="title-row" style="color: var(--accent);">📋 ${displayTitle.toUpperCase()}</div>
...
async function sendCmd(event) {
    ...
    if (!isExtraCmd && !isWeightCmd) {
        ...
        if (grid) {
            ...
            if (lastPinned) lastPinned.after(tempCard);
            else grid.prepend(tempCard);
        }
    }
...
function openNoteByIndex(index) {
    ...
    if (note.type === 'LINK') {
        ...
        linkView.innerHTML = `
            ${imageUrl ? `<div style="width: 100%; height: 200px; background: url('${imageUrl}'); background-size: cover; background-position: center; border-radius: 12px; border: 1px solid var(--accent);"></div>` : ''}
            <div style="background: var(--bg); padding: 20px; border-radius: 12px; border-left: 3px solid var(--accent);">
                <div style="font-size: 10px; color: var(--dim-2); margin-bottom: 5px; letter-spacing: 1px; text-transform: uppercase;">Link Salvato</div>
                <a href="${url}" target="_blank" style="color: var(--accent); font-size: 14px; text-decoration: none; word-break: break-all; display: block; margin-bottom: 15px;">${url}</a>
                <button onclick="window.open('${url}', '_blank')" style="background: var(--accent); color: #fff; border: none; padding: 12px 24px; border-radius: 8px; cursor: pointer; font-family: 'Space Grotesk'; font-weight: 600; width: 100%;">APRI LINK ↗</button>
            </div>
            ${description ? `<div style="color: var(--dim); font-size: 13px; line-height: 1.6; padding: 15px; background: var(--glass); border-radius: 8px; border: 1px solid var(--border);">${description}</div>` : ''}
        `;
...
function openExtraDetail(categoria) {
    ...
    try {
        ...
        if (filteredExtra.length === 0) {
            ...
            let html = `
                <div style="margin-bottom:15px; padding:15px; background:var(--glass); border:1px solid var(--accent); border-radius:12px; text-align:center; backdrop-filter:var(--blur);">
                    <span style="font-size:10px; opacity:0.6; display:block; letter-spacing:1px; text-transform:uppercase;">Totale Ore</span>
                    <span style="font-size:24px; color:var(--accent); font-weight:700">${monthTotal.toFixed(1)}h</span>
                </div>`;
            ...
            html += filteredExtra.map(item => `
                <div style="display:flex; justify-content:space-between; align-items:center; padding:12px; border-bottom:1px solid var(--border);">
                    ...
                    <span style="font-size:10px; color:var(--dim-2);">${item.nota || ''}</span>
                </div>`).join('');
            list.innerHTML = html;
        }
    } catch (err) {
...
function closeModal() { closeNoteDetail(false); }
...
function changeNoteColor(color) {
    ...
    if (card) card.className = `keep-card bg-${color}${currentNoteData.type === 'PINNED' ? ' pinnato' : ''}`;
}
...
async function togglePinFromCard(id) {
    ...
    await fetch(SCRIPT_URL, {
        method: 'POST', mode: 'no-cors',
        body: JSON.stringify({ service: "update_note_type", id: id, type: newType })
    });
}
...
async function executeDelete() {
    ...
    if (indexToRemove !== -1) loadedNotesData.splice(indexToRemove, 1);
...
function toggleBrainSearch() {
    ...
    if (overlay.style.display === 'none' || !overlay.style.display) {
        ...
        setTimeout(() => { overlay.style.transform = 'translateY(0)'; input.focus(); }, 10);
    } else {
...
function filterNotes(query) {
    ...
    if (!searchTerm) { allCards.forEach(card => card.style.display = 'flex'); return; }
...
async function handleFinanceSubmit(event) {
    ...
    try {
        ...
        if (textLower.includes('*cash') || (textLower.includes('cash') && !textLower.includes('cashback'))) targetWallet = "CASH";
        else if (textLower.includes('*tin') || textLower.includes('tinaba')) targetWallet = "TINABA";
...
async function submitFinanceSingle() {
    ...
    const text = `${financeSign * Math.abs(amount)} ${desc}${note ? ', ' + note : ''}`;
    await fetch(SCRIPT_URL, {
        method: 'POST', mode: 'no-cors',
...
let financeSign = -1; // default spesa

function toggleFinanceSign() {
    financeSign = financeSign * -1;
    const btn = document.getElementById('fin-sign-btn');
    if (financeSign === 1) {
        btn.textContent = '+';
...
async function submitFinanceMulti() {
    ...
    btn.style.color = '#888';
    rows.forEach(row => {
    const signBtn = row.querySelector('button');
    ...
    if (amount && desc) entries.push({ amount: sign * Math.abs(amount), desc, wallet });
    });
...
function toggleFilters(show) {
    ...
    if(show) {
        ...
        input.value = '';
        input.focus();
        container.innerHTML = `<div style="text-align:center; color:var(--dim-2); margin-top:30px; font-size:12px;">DIGITA E PREMI INVIO PER CERCARE NEL DATABASE COMPLETO</div>`;
    } else {
        overlay.style.display = 'none';
        input.blur();
    }
...
function renderFilteredItems(items) {
    const container = document.getElementById('filtered-results');
    if (items.length === 0) {
        container.innerHTML = "<div style='color:var(--dim-2); text-align:center; margin-top:20px;'>NO_DATA_FOUND</div>";
        return;
    }
    container.innerHTML = items.map(t => {
        const iconName = financeIcons[t.cat] || "arrow-right-left";
        const hasNote = t.note && t.note !== "";
        const color = t.amt < 0 ? "var(--text)" : "#4ade80";
        return `
...
function quickFilter(tag) {
    ...
    if (!currentVal.includes(tag)) input.value = currentVal ? `${currentVal} ${tag}` : tag;
...
function toggleAISearch() {
    ...
    status.style.color = aiSearchActive ? 'var(--accent)' : 'var(--dim)';
    box.style.borderColor = aiSearchActive ? 'var(--accent)' : 'var(--border)';
}
...
function switchPage(pageId) {
    ...
    if (pageId === 'log' && lastStatsData) renderFilteredItems(lastStatsData.finance.transactions);
    if (window.lucide) lucide.createIcons();
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (backdrop) backdrop.onclick = () => closeNoteDetail(true);
    if (closeBtn) closeBtn.onclick = () => closeNoteDetail(true);
});
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        ...
        if (weightModal && weightModal.style.display !== 'none') closeWeightLog();
        const workoutModal = document.getElementById('workout-feeling-modal');
        if (workoutModal && workoutModal.style.display === 'block') closeWorkoutFeeling();
    }
});
...
function toggleStats() {
    ...
    if (!cachedFinanceStats) { showCustomAlert("DATI_NON_DISPONIBILI"); return; }
    ...
    financeChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                legend: { 
                    position: 'bottom', 
                    labels: { color: '#888', font: { family: 'Space Grotesk', size: 10 } } 
                }
            }
        }
    });
...
async function createNew(type) {
    ...
    if (noteDetail && noteDetail.style.display === 'flex') closeNoteDetail(false);
    ...
    if (type === 'NOTE') {
        ...
        if (todoContainer) todoContainer.style.display = 'none';
        if (detailText) { detailText.style.display = 'block'; detailText.value = ""; }
...
function renderTodoItems() {
    ...
    container.innerHTML = todoItems.map(item => `
        <div style="display: flex; align-items: center; gap: 12px; padding: 12px; background: var(--glass); margin-bottom: 8px; border-radius: 8px; border: 1px solid var(--border);">
            <div onclick="toggleTodoItem('${item.id}')" style="min-width: 20px; width: 20px; height: 20px; border: 2px solid ${item.checked ? 'var(--accent)' : 'var(--border)'}; background: ${item.checked ? 'var(--accent)' : 'transparent'}; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s;">
                ${item.checked ? '<i data-lucide="check" style="width: 14px; color: #fff;"></i>' : ''}
            </div>
            <span style="flex: 1; font-family: 'JetBrains Mono'; font-size: 13px; ${item.checked ? 'text-decoration: line-through; opacity: 0.4;' : 'color: var(--text);'}">${item.text}</span>
            <i data-lucide="x" style="width: 16px; color: var(--dim); cursor: pointer; opacity: 0.5;" onclick="removeTodoItem('${item.id}')"></i>
        </div>
    `).join('');
    if(window.lucide) lucide.createIcons();
}
...
async function saveTodoList() {
    if (todoItems.length === 0) { showCustomAlert("ADD_AT_LEAST_ONE_ITEM"); return; }
    ...
    const fakeId = 'temp_' + Date.now();
    const fakeNote = { id: fakeId, date: new Date(), type: 'LISTA', content: todoText, color: 'default', title: listTitle || 'TODO_LIST' };
    loadedNotesData.unshift(fakeNote);
    lastStatsData.notes = loadedNotesData;
    ...
    if (grid) {
        ...
        const cardHTML = `
            <div class="keep-card bg-default" id="card-${fakeId}" style="cursor: pointer;">
                <div class="title-row" style="color: var(--accent);">📋 ${(listTitle || 'TODO_LIST').toUpperCase()}</div>
                <div class="content-preview">${todoText.substring(0, 100)}</div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;">
                    <div class="label" style="font-size:9px; opacity:0.4;">JUST_NOW</div>
                    <div style="font-size: 10px; color: var(--accent);">${checkedItems}/${totalItems} ✓</div>
                </div>
            </div>`;
        const lastPinned = grid.querySelector('.pinnato:last-of-type');
        if (lastPinned) lastPinned.insertAdjacentHTML('afterend', cardHTML);
        else grid.insertAdjacentHTML('afterbegin', cardHTML);
    }
    
    closeTodoModal();
    try {
        await fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ service: "note", text: todoText }) });
...
function renderInteractiveTodo(note) {
    ...
    const titleHTML = titleLine ? `
        <input type="text" id="todo-title-edit" value="${titleLine.replace(/"/g, '&quot;')}" placeholder="NOME LISTA..."
            style="width:100%; background:transparent; border:none; border-bottom:1px solid var(--border); color:var(--accent); font-family:'Space Grotesk'; font-size:1rem; font-weight:700; letter-spacing:-0.3px; padding:8px 0; margin-bottom:12px; outline:none; box-sizing:border-box;"
            onchange="saveTodoState()">
    ` : `
        <input type="text" id="todo-title-edit" value="" placeholder="Aggiungi titolo..."
            style="width:100%; background:transparent; border:none; border-bottom:1px solid var(--border); color:var(--dim); font-family:'Space Grotesk'; font-size:0.9rem; letter-spacing:-0.3px; padding:6px 0; margin-bottom:12px; outline:none; box-sizing:border-box;"
            onchange="saveTodoState()">
    `;
    ...
    todoContainer.innerHTML = titleHTML + items.map(item => {
        const textWithLinks = item.text.replace(urlRegex, (url) => {
            ...
            return `<a href="${url}" target="_blank" onclick="event.stopPropagation()" style="color:var(--accent); text-decoration:none; font-size:11px;">↗ ${domain}</a>`;
        });
        return `
        <div style="display: flex; align-items: flex-start; gap: 12px; padding: 12px; background: var(--glass); margin-bottom: 8px; border-radius: 8px; border: 1px solid var(--border);">
            <div onclick="toggleSavedTodo(this, '${item.id}')" data-checked="${item.checked}"
                style="min-width: 20px; width: 20px; height: 20px; margin-top: 2px; border: 2px solid ${item.checked ? 'var(--accent)' : 'var(--border)'}; background: ${item.checked ? 'var(--accent)' : 'transparent'}; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center;">
                ${item.checked ? '<i data-lucide="check" style="width: 14px; color: #fff;"></i>' : ''}
            </div>
            <span data-text="${item.text.replace(/"/g, '&quot;')}"
                style="flex: 1; font-family: 'JetBrains Mono'; font-size: 13px; line-height: 1.5; ${item.checked ? 'text-decoration: line-through; opacity: 0.4;' : 'color: var(--text);'}">${textWithLinks}</span>
        </div>`;
    }).join('');
    if(window.lucide) lucide.createIcons();
}
...
function toggleSavedTodo(checkbox, id) {
    ...
    checkbox.style.borderColor = newChecked ? 'var(--accent)' : 'var(--border)';
    const textSpan = checkbox.nextElementSibling;
    if (newChecked) {
        checkbox.innerHTML = '<i data-lucide="check" style="width: 14px; color: #fff;"></i>';
...
async function saveTodoState() {
    ...
    await fetch(SCRIPT_URL, {
        method: 'POST', mode: 'no-cors',
        body: JSON.stringify({ service: 'update_note', id: currentNoteData.id, text: newContent })
    });
}
...
function toggleGhostAI() {
    ...
    if (qMenu && !qMenu.classList.contains('quick-menu-hidden')) qMenu.classList.add('quick-menu-hidden');
    const bubble = document.getElementById('analyst-bubble');
    const text = document.getElementById('analyst-text');
    if (!bubble || !text) return;
    bubble.classList.remove('active');
    void bubble.offsetWidth;
    text.innerHTML = `
        <div style="font-size: 0.7rem; color: var(--dim-2); margin-bottom: 8px; letter-spacing: 2px; text-transform: uppercase;">Ghost AI · Brain Module</div>
        ...
        <div style="display: flex; gap: 8px; margin-top: 10px;">
            <input type="text" id="ghost-input" placeholder="Descrivi cosa vuoi fare..." 
                style="flex:1; background:var(--glass); border:1px solid var(--border); color:var(--text); padding:8px 12px; font-family:'JetBrains Mono'; font-size:0.8rem; outline:none; border-radius:8px;"
                onkeypress="handleGhostInput(event)">
        </div>
        <div style="font-size: 0.65rem; color:var(--dim-2); margin-top:8px;">
            es: "espandi questa idea", "correggi grammatica", "crea lista da questo testo"
        </div>
    `;
    bubble.classList.add('active');
    setTimeout(() => { const ghostInput = document.getElementById('ghost-input'); if (ghostInput) ghostInput.focus(); }, 200);
}
...
async function handleGhostInput(event) {
    ...
    setTimeout(() => {
        text.innerHTML = `
            <div style="font-size:0.7rem; color:var(--dim-2); text-transform:uppercase; letter-spacing:1px;">Ghost Response:</div>
            <div style="color:var(--text); margin-top:8px;">Gemini non ancora collegato.<br>
...
function renderStars(rating, color) {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        else starsHtml += `<i data-lucide="star" style="color:var(--border); width:16px; height:16px;"></i>`;
    }
    return starsHtml;
}
...
async function processReviewWithAI() {
    ...
    loadingCard.innerHTML = `
        <div class="poster-mini" style="background: var(--glass);">
    ...
    try {
        const response = await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ service: 'processReviewAI', text: input }) });
        ...
        if (result.status === "SUCCESS") {
            ...
            loadingCard.innerHTML = `
                ...
                <div class="review-info">
                    <div class="review-top">
                        <span class="review-title" style="color: var(--accent);">${(ai.titolo || "").toUpperCase()}</span>
                        <span class="rating-stars">${renderStars(ai.rating, 'var(--accent)')}</span>
                    </div>
                    <div class="review-comment">${ai.commento_breve || ""}</div>
                    <div class="review-meta"><span>${ai.categoria}</span><span>${new Date().toLocaleDateString('it-IT', {day: '2-digit', month: 'short'})}</span></div>
                </div>`;
            if (window.lucide) lucide.createIcons();
            setTimeout(() => loadStats(), 2000);
        } else {
            loadingCard.innerHTML = `<div style="padding:10px; color:#ff4d6d;">ERRORE: ${result.message || 'SYNC_FAILED'}</div>`;
        }
    } catch (error) {
        loadingCard.innerHTML = `<div style="padding:10px; color:#ff4d6d;">ERRORE_CONNESSIONE</div>`;
    }
}
...
function generateStatsHTML(period = '6M', filterCat = 'ALL') {
    ...
    const colors = { FILM: '#3b82f6', SERIE: '#8b5cf6', GAME: '#10b981', COMIC: '#f59e0b' };
    let trendData = {};
    let trendHTML = '';
    const now = new Date();
        ...
        trendHTML = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; height: 120px;">
                ${Object.entries(counts).map(([cat, val]) => `
                    <div style="background: var(--glass); border: 1px solid var(--border); padding: 10px; border-radius: 8px; display: flex; flex-direction: column; justify-content: center; opacity: ${isAll || filterCat === cat ? 1 : 0.2}">
                        <div style="font-size: 8px; color: ${colors[cat]}; letter-spacing: 1px; text-transform: uppercase;">${cat}</div>
                        <div style="font-size: 20px; font-family: 'Space Grotesk'; font-weight: 700; color: var(--text);">${val}</div>
                    </div>
                `).join('')}
            </div>`;
    } else {
                ...
                ${Object.entries(trendData).map(([label, val]) => {
                    ...
                    return `
                        <div style="flex:1; display:flex; flex-direction:column; align-items:center; gap:5px; height:100%; justify-content:flex-end;">
                            ...
                            <div style="width:100%; height:${h}%; background:var(--accent); opacity:${val > 0 ? 0.6 : 0.05}; border-radius:2px;"></div>
                            <div style="font-size:7px; color:var(--dim-2); transform:rotate(-45deg); margin-top:10px; white-space:nowrap;">${label}</div>
                        </div>`;
                }).join('')}
            </div>`;
    }
    ...
    container.innerHTML = `
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin-bottom: 20px;">
            <div style="background:var(--glass); border:1px solid var(--border); padding:12px; border-radius:12px; text-align:center; backdrop-filter:var(--blur);">
                <div style="font-size:8px; color:var(--dim-2); margin-bottom:5px; text-transform:uppercase; letter-spacing:1px;">LOGS</div>
                <div style="font-size:18px; font-family:'Space Grotesk'; font-weight:700; color:var(--text);">${activeReviews.length}</div>
            </div>
            <div style="background:var(--glass); border:1px solid var(--border); padding:12px; border-radius:12px; text-align:center; backdrop-filter:var(--blur);">
                <div style="font-size:8px; color:var(--dim-2); margin-bottom:5px; text-transform:uppercase; letter-spacing:1px;">SCORE AVG</div>
                <div style="font-size:18px; font-family:'Space Grotesk'; font-weight:700; color:#f59e0b;">${avgRating}</div>
            </div>
            <div style="background:var(--glass); border:1px solid var(--border); padding:12px; border-radius:12px; text-align:center; backdrop-filter:var(--blur);">
                <div style="font-size:8px; color:var(--dim-2); margin-bottom:5px; text-transform:uppercase; letter-spacing:1px;">WISH</div>
                <div style="font-size:18px; font-family:'Space Grotesk'; font-weight:700; color:var(--accent);">${categoryWishCount}</div>
            </div>
        </div>

        <div style="background:var(--glass); border:1px solid var(--border); padding:15px; border-radius:12px; margin-bottom:20px; backdrop-filter:var(--blur);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                <h3 style="font-family:'Space Grotesk'; font-size:10px; color:var(--dim-2); letter-spacing:1px; margin:0; text-transform:uppercase;">${filterCat} ACTIVITY</h3>
                <div style="display:flex; gap:10px; font-size:9px; font-family:'JetBrains Mono';">
                    <span onclick="generateStatsHTML('6M', '${filterCat}')" style="cursor:pointer; color:${period==='6M'?'var(--accent)':'var(--dim-2)'}">6M</span>
                    <span onclick="generateStatsHTML('1Y', '${filterCat}')" style="cursor:pointer; color:${period==='1Y'?'var(--accent)':'var(--dim-2)'}">1Y</span>
                    <span onclick="generateStatsHTML('ALL', '${filterCat}')" style="cursor:pointer; color:${period==='ALL'?'var(--accent)':'var(--dim-2)'}">ALL</span>
                </div>
            </div>
            ${trendHTML}
        </div>

        <div style="background:var(--glass); border:1px solid var(--border); padding:15px; border-radius:12px; display: ${isAll ? 'block' : 'none'}; backdrop-filter:var(--blur);">
            <h3 style="font-family:'Space Grotesk'; font-size:10px; color:var(--dim-2); margin-bottom:15px; letter-spacing:1px; text-transform:uppercase;">MEDIA DISTRIBUTION</h3>
            ${Object.entries(counts).map(([cat, count]) => {
                const p = activeReviews.length ? (count / activeReviews.length * 100) : 0;
                return `
                    <div style="margin-bottom:12px;">
                        <div style="display:flex; justify-content:space-between; font-size:9px; margin-bottom:4px; font-family:'JetBrains Mono'; color:var(--dim);">
                        ...
                        <div style="width:100%; height:3px; background:var(--border); border-radius:2px; overflow:hidden;">
                            <div style="width:${p}%; height:100%; background:${colors[cat]}; border-radius:2px; transition: width 0.6s ease;"></div>
                        </div>
                    </div>`;
            }).join('')}
        </div>`;
...
async function loadBodyData() {
    if (!lastStatsData || !lastStatsData.body) return;
...
function renderBodyDashboard() {
    ...
    if (weightEl && bodyData.currentWeight) weightEl.innerText = bodyData.currentWeight.toFixed(1);
    ...
    if (deltaEl) {
        if (bodyData.weightHistory && bodyData.weightHistory.length > 1) {
            ...
            const color = delta < 0 ? '#4ade80' : delta > 0 ? '#ff4d6d' : 'var(--dim)';
            deltaEl.innerHTML = `<span style="color:${color};">${arrow} ${Math.abs(delta).toFixed(1)} kg</span>`;
        } else {
            deltaEl.innerHTML = '<span style="color:var(--dim);">Aggiungi più dati peso</span>';
        }
    }
    
    const streakEl = document.getElementById('body-streak');
    if (streakEl) streakEl.innerText = calculateStreak(bodyData.workouts);
...
function calculateStreak(workouts) {
    ...
    if ((weeklyWorkouts[currentWeekKey] || 0) < TARGET_WORKOUTS_PER_WEEK) currentDate.setDate(currentDate.getDate() - 7);
    while (true) {
...
function renderTodayLog() {
    ...
    const todayWorkout = bodyData.workouts.find(w => new Date(w.date).toISOString().split('T')[0] === todayStr);
    ...
    if (todayWorkout) {
        const exCount = Array.isArray(todayWorkout.exercises) && typeof todayWorkout.exercises[0] === 'object' 
            ? todayWorkout.exercises.length : 'Completato';
        todayLogs.push({ 
            type: 'workout', 
    ...
    if (todayLogs.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:var(--dim-2); padding:20px; font-size:10px; letter-spacing:1px;">NESSUN_LOG_OGGI</div>';
        return;
    }
    ...
    container.innerHTML = todayLogs.map(log => {
        ...
        return `
            <div style="display: flex; align-items: center; gap: 12px; padding: 10px 0; border-bottom: 1px solid var(--border);">
                ...
                <div style="flex: 1;">
                    <div style="font-size: 0.85rem; color: var(--text);">${log.text}</div>
                    <div style="font-size: 0.7rem; color: var(--dim-2); margin-top: 2px;">${log.time}</div>
                </div>
                <div style="font-size: 1rem; color: #4ade80;">✓</div>
            </div>`;
    }).join('');
}
...
function renderRecentWorkouts() {
    ...
    if (!bodyData.workouts || bodyData.workouts.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:var(--dim-2); padding:20px; font-size:10px;">NESSUN_WORKOUT_REGISTRATO</div>';
        return;
    }
    ...
    const formattedText = textContent
        .replace(/;/g, '<br>')
        .replace(/\(↑\)/g, '<b style="color: #4ade80;">↑</b>')
        .replace(/\(↓\)/g, '<b style="color: #ff4d6d;">↓</b>')
        .replace(/\(=\)/g, '<b style="color: var(--dim);">=</b>')
        .replace(/\(new\)/g, '<b style="color: var(--accent);">NEW</b>');
    
    container.innerHTML = `
        <div style="background: var(--glass); padding: 15px; border-radius: 12px; border-left: 3px solid var(--accent); border: 1px solid var(--border); backdrop-filter: var(--blur);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                <div>
                    <div style="font-size: 0.7rem; color: var(--dim-2); font-family: 'JetBrains Mono'; margin-bottom: 2px; letter-spacing: 1px; text-transform: uppercase;">Data Sessione</div>
                    <div style="font-size: 0.85rem; color: var(--text); font-family: 'JetBrains Mono';">${dateStr}</div>
                </div>
                <div style="font-size: 1.5rem;">${moodEmoji[String(lastW.mood)] || '😊'}</div>
            </div>
            <div style="color: var(--dim); font-size: 0.85rem; line-height: 1.6; font-family: 'JetBrains Mono'; margin-bottom: 10px;">
                ${formattedText.split('<br>').slice(0, 3).join('<br>')}
                ${formattedText.split('<br>').length > 3 ? '<div style="color:var(--dim-2); font-size:0.7rem; margin-top:4px;">...e altri esercizi nella History</div>' : ''}
            </div>
            <div style="display: flex; gap: 15px; border-top: 1px solid var(--border); margin-top: 10px; padding-top: 10px;">
                <div style="font-size: 0.7rem; color: var(--dim);">
                    <span style="color: var(--accent);">ENERGY:</span> ${String(lastW.energy).toUpperCase()}
                </div>
                <div style="font-size: 0.7rem; color: var(--dim);">
                    <span style="color: var(--accent);">TIME:</span> ${lastW.duration || '--'} MIN
                </div>
            </div>
        </div>`;
...
function selectWorkoutMood(value, btn) {
    selectedMood = value;
    document.querySelectorAll('#mood-selector button').forEach(b => b.style.borderColor = 'var(--border)');
    btn.style.borderColor = '#4ade80';
}

function selectWorkoutEnergy(value, btn) {
    selectedEnergy = value;
    document.querySelectorAll('#energy-selector button').forEach(b => { b.style.borderColor = 'var(--border)'; b.style.color = 'var(--dim)'; });
...
async function submitWorkoutFeeling() {
    ...
    btn.disabled = true;
    try {
        await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ service: 'save_workout', raw_input: input, manual_mood: selectedMood, manual_energy: selectedEnergy }) });
        const coachRes = await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'ask_body_coach', query: input, mood: selectedMood, energy: selectedEnergy }) });
        ...
        coachZone.innerHTML = `
            <p style="color:#4ade80; font-family:'JetBrains Mono'; font-size:0.85rem; line-height:1.4;">${coachText}</p>
            <button onclick="location.reload()" style="width:100%; margin-top:10px; padding:10px; background:transparent; border:1px solid var(--accent); color:var(--accent); font-family:'Space Grotesk'; font-weight:600; cursor:pointer; border-radius:8px;">CHIUDI SESSIONE</button>`;
    } catch (e) {
...
async function submitWeight() {
    ...
    if (!weight || weight < 30 || weight > 300) { showCustomAlert("PESO_NON_VALIDO"); return; }
    input.disabled = true;
    const originalValue = input.value;
    input.value = "SAVING...";
    try {
        await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'log_weight', weight: weight }) });
        lastStatsData.body.weight = weight;
        bodyData.currentWeight = weight;
        if (bodyData.weightHistory) bodyData.weightHistory.push({ date: new Date().toISOString(), weight: weight });
        ...
        setTimeout(() => { closeWeightLog(); if (typeof renderBodyDashboard === 'function') renderBodyDashboard(); }, 1500);
    } catch (e) {
...
function toggleBodyCoach() {
    ...
    bubble.classList.add('active');
    if (navItem) navItem.style.color = '#4ade80';
    text.innerHTML = `
        <div style="font-size: 0.7rem; color: var(--dim-2); margin-bottom: 8px; letter-spacing: 2px; font-family: 'Space Grotesk'; text-transform: uppercase;">Coach AI · Ready</div>
        <div style="color: #4ade80; font-size: 0.9rem; margin-bottom: 12px; font-family: 'JetBrains Mono';">Che c'è? Hai bisogno di una spinta o vuoi solo lamentarti?</div>
        <input type="text" id="coach-input" placeholder="Scrivi qui..." 
               style="width: 100%; background: var(--glass); border: 1px solid var(--border); color: var(--text); padding: 12px; font-family: 'JetBrains Mono'; font-size: 0.85rem; outline: none; border-radius: 8px; box-sizing: border-box;"
...
async function handleCoachInput(event) {
    ...
    if (!query) return;
    const text = document.getElementById('body-coach-text');
    text.innerHTML = `<div style="font-size: 0.7rem; color: var(--dim-2); margin-bottom: 8px; letter-spacing: 2px; text-transform: uppercase;">Coach AI · Analyzing</div>
                      <div style="color: #4ade80;" class="blink">PENSANDO... (Sii pronto a piangere)</div>`;
    try {
        const response = await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ action: 'ask_body_coach', query: query }) });
        const coachResponse = await response.text();
        text.innerHTML = `
            <div style="font-size: 0.7rem; color: var(--dim-2); margin-bottom: 6px; text-transform: uppercase; letter-spacing: 1px;">Coach AI · Response:</div>
            <div style="color: var(--text); line-height: 1.6; font-family: 'JetBrains Mono'; font-size: 0.9rem; margin-bottom: 15px;">"${coachResponse}"</div>
            ...
            <div onclick="document.getElementById('body-coach-bubble').classList.remove('active'); document.getElementById('body-nav-ai').style.color = 'var(--dim)';" 
                 style="margin-top: 15px; font-size: 0.7rem; color: var(--dim-2); cursor: pointer; text-align: right; font-family: 'JetBrains Mono';">
                [CHIUDI SESSIONE]
            </div>`;
        setTimeout(() => document.getElementById('coach-input')?.focus(), 100);
    } catch (e) {
        text.innerHTML = `<div style="color: #ff4d6d;">ERRORE_CONNESSIONE_COACH</div>
                          <div onclick="toggleBodyCoach()" style="cursor:pointer; color:var(--text); font-size:0.7rem; margin-top:10px;">[RIPROVA]</div>`;
    }
}

function switchBodyView(view) {
    if (currentBodyView === view) view = 'dashboard';
    ...
    if (view === 'dashboard') {
        ...
        document.getElementById('body-nav-stats').style.color = 'var(--accent)'; 
        renderBodyCharts(); 
    } else if (view === 'history') {
        document.getElementById('body-history-view').style.display = 'block';
        document.getElementById('body-nav-history').style.color = 'var(--accent)';
        renderBodyHistory();
    }
}
...
function initBodyModule() {
    if (!lastStatsData) {
        setTimeout(() => {
            if (lastStatsData) { loadBodyData(); switchBodyView('dashboard'); }
            else {
                const dashboard = document.getElementById('body-dashboard');
                if (dashboard) dashboard.innerHTML = '<div style="text-align:center; padding:40px; color:var(--dim);" class="blink">SYNCING_BODY_DATA...</div>';
            }
        }, 1000);
    } else {
        loadBodyData();
...
function renderBodyCharts() {
    ...
    if (weightCtx && bodyData.weightHistory) {
        ...
        window.weightChartInstance = new Chart(weightCtx, {
            ...
            data: {
                labels: wLabels,
                datasets: [{ data: wValues, borderColor: 'var(--accent)', backgroundColor: 'rgba(59,130,246,0.08)', borderWidth: 2, fill: true, tension: 0.3, pointRadius: 3 }]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                plugins: { legend: { display: false } },
                scales: {
                    y: { grid: { color: 'rgba(255,255,255,0.04)' }, ticks: { color: '#555', font: { size: 9 } } },
        ...
        if (filteredWorkouts.length > 0) {
            ...
            window.workoutChartInstance = new Chart(workoutCtx, {
                data: {
                    labels: labels,
                    datasets: [
                        { type: 'line', label: 'Energia', data: energyValues, borderColor: 'var(--accent)', borderWidth: 2, pointRadius: 2, tension: 0.4, order: 1 },
                        { type: 'bar', label: 'Mood', data: moodValues, backgroundColor: moodValues.map(v => v > 2 ? '#4ade80' : v < 2 ? '#ff4d6d' : '#444'), borderRadius: 4, order: 2 },
                        { type: 'bar', label: 'Capsula', data: backgroundFill, backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 4, order: 3 }
                    ]
                },
                options: {
                    responsive: true, maintainAspectRatio: false,
...
function renderTopImprovements() {
    ...
    if (!topContainer) {
        ...
        topContainer.style.cssText = "background: var(--glass); border: 1px solid var(--border); padding: 20px; border-radius: 12px; margin-top: 20px; backdrop-filter: var(--blur);";
    ...
    [...bodyData.workouts].reverse().forEach(w => {
        ...
        items.forEach(item => {
            ...
            if (match) {
                ...
                if (!stats[name]) stats[name] = { first: weight, last: weight, count: 0 };
    ...
    const top3 = Object.entries(stats)
        .map(([name, data]) => ({ name, diff: data.last - data.first, last: data.last, score: data.count }))
        .filter(ex => ex.diff > 0 || ex.score > 0)
        .sort((a, b) => b.diff - a.diff || b.score - a.score)
        .slice(0, 3);

    if (top3.length === 0) {
        topContainer.innerHTML = `<h3 style="font-family:'Space Grotesk'; font-size:0.9rem; color:var(--dim); margin:0; font-weight:600;">TOP PROGRESSION</h3>
                                  <div style="color:var(--dim-2); font-size:0.8rem; margin-top:10px;">Dati insufficienti per il calcolo progressione.</div>`;
        return;
    }
    topContainer.innerHTML = `
        <h3 style="font-family:'Space Grotesk'; font-size:0.9rem; color:var(--dim); margin:0 0 15px 0; font-weight:600; letter-spacing:-0.3px;">TOP PROGRESSION</h3>
        ${top3.map(ex => `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; padding:10px; background:var(--glass); border-radius:8px; border:1px solid var(--border);">
                <div style="flex:1;">
                    <div style="color:var(--text); font-size:0.85rem; font-weight:500;">${ex.name}</div>
                    <div style="color:var(--dim-2); font-size:0.7rem;">Max attuale: ${ex.last}kg</div>
                </div>
                <div style="text-align:right;">
                    <div style="color:#4ade80; font-family:'Space Grotesk'; font-weight:700; font-size:1rem;">${ex.diff > 0 ? `+${ex.diff}kg` : 'STABILE'}</div>
                    <div style="font-size:0.6rem; color:#4ade80; opacity:0.7;">GAINS DETECTED</div>
                </div>
            </div>
        `).join('')}
    `;
...
function renderBodyHistory() {
    ...
    const avgMood = workouts.length > 0 
        ? (workouts.reduce((acc, curr) => acc + Number(curr.mood || 0), 0) / workouts.length).toFixed(1) : 0;
    avgMoodSpan.innerText = (avgMood > 0 ? '+' : '') + avgMood;
    const avgTime = workouts.length > 0
        ? Math.round(workouts.reduce((acc, curr) => acc + Number(curr.duration || 0), 0) / workouts.length) : 0;
    ...
    workouts.forEach(w => {
        ...
        const exercises = rawText.split(';').map(ex => {
            return ex.trim()
                .replace(/\(new\)/g, '<i data-lucide="sparkles" style="width:14px;height:14px;color:var(--accent);display:inline-block;margin:0 3px;"></i><span style="color:var(--accent); font-weight:600;">NEW</span>')
                .replace(/\(↑\)/g, '<i data-lucide="trending-up" style="width:14px;height:14px;color:#4ade80;display:inline-block;margin:0 3px;"></i>')
                .replace(/\(↓\)/g, '<i data-lucide="trending-down" style="width:14px;height:14px;color:#ff4d6d;display:inline-block;margin:0 3px;"></i>')
                .replace(/\(=\)/g, '<i data-lucide="equal" style="width:14px;height:14px;color:var(--dim);display:inline-block;margin:0 3px;"></i>');
        }).filter(Boolean);
        const detailText = exercises.map(ex => `• ${ex}`).join('<br>');
        const card = document.createElement('div');
        card.style.cssText = `background: var(--glass); border: 1px solid var(--border); border-left: 3px solid ${Number(w.mood) >= 1 ? '#4ade80' : Number(w.mood) <= -1 ? '#ff4d6d' : 'var(--border)'}; padding: 15px; border-radius: 12px; margin-bottom: 12px; backdrop-filter: var(--blur);`;
        card.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    <span style="font-family: 'JetBrains Mono'; font-size: 0.8rem; color: var(--text); background: var(--glass); border: 1px solid var(--border); padding: 2px 8px; border-radius: 6px;">${day.toUpperCase()}</span>
                ...
                <span style="font-family: 'JetBrains Mono'; font-size: 0.7rem; color: var(--accent);">${durationVal} MIN</span>
            </div>
            <div style="font-family: 'JetBrains Mono'; font-size: 0.8rem; color: var(--dim); line-height: 1.5; margin-bottom: 10px; padding-left: 5px;">
                ${detailText !== "Dettaglio non trovato" ? detailText : `<span style="color: var(--dim-2);">Dettaglio non trovato</span>`}
            </div>
            ${w.notes ? `<div style="border-top: 1px solid var(--border); padding-top: 8px; margin-top: 5px; font-size: 0.7rem; color: var(--dim-2); font-style: italic;">${w.notes}</div>` : ''}
        `;
...
function renderWithData(data) {
    if (!data) { console.warn("renderWithData chiamato senza dati"); return; }
    ...
    if (document.getElementById('body')?.classList.contains('active')) loadBodyData();
    renderGrid(data);
    ...
    if (data.finance) {
        ...
        if (widgetSpent) {
            ...
            if (income > 0) {
                ...
                if (spentPercent > 85) color = '#ff4d6d';
                else if (spentPercent > 60) color = '#fb923c';
                else color = '#4ade80';
            } else if (spent > 0) color = '#ff4d6d';
        ...
        if (icon) { icon.setAttribute('data-lucide', balanceHidden ? 'eye-off' : 'eye'); if (window.lucide) lucide.createIcons(); }
        ...
        if (fill && infoText) {
            if (inc > 0) {
                ...
                fill.style.background = lifePct < 20 ? "#ff4d6d" : "var(--accent)";
        ...
        if (data.finance.transactions) renderFinanceLog(data.finance.transactions);
        if (data.finance) cachedFinanceStats = calculateFinanceStats(data.finance);
    }
}
...
async function fetchLinkPreview() {
    ...
    if (!url) return;
    try { new URL(url); } catch(e) { showCustomAlert("URL_NON_VALIDO"); return; }
    input.style.borderColor = 'var(--accent)';
    input.disabled = true;
    try {
        ...
        const data = await response.json();
        if (data.title) {
            currentLinkData = { url, title: data.title, description: data.description || '', image: data.image || '', domain: new URL(url).hostname };
            ...
            if (data.image) {
                ...
                imgEl.style.background = 'var(--glass)';
                imgEl.innerHTML = '<div style="display:flex; align-items:center; justify-content:center; height:100%; color:var(--dim-2); font-size:3rem;">🔗</div>';
            }
...
async function saveLinkNote() {
    ...
    const fakeId = 'temp_' + Date.now();
    const fakeNote = { id: fakeId, date: new Date(), type: 'LINK', content: linkText, color: 'default', title: currentLinkData.title };
    loadedNotesData.unshift(fakeNote);
    lastStatsData.notes = loadedNotesData;
    const grid = document.getElementById('keep-grid');
    if (grid) {
        const cardHTML = `
            <div class="keep-card bg-default" id="card-${fakeId}" style="cursor: pointer;">
                <div style="width: 100%; height: 80px; background: ${currentLinkData.image ? `url('${currentLinkData.image}')` : 'var(--glass)'}; background-size: cover; background-position: center; border-radius: 8px; margin-bottom: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; border: 1px solid var(--accent);">${currentLinkData.image ? '' : '🔗'}</div>
                <div class="title-row" style="color: var(--accent);">${currentLinkData.title.toUpperCase()}</div>
                <div class="content-preview" style="font-size: 10px;">${currentLinkData.description.substring(0, 80)}</div>
                <div style="font-size: 9px; color: var(--accent); margin-top: 8px; opacity: 0.6;">↗ ${currentLinkData.domain}</div>
            </div>`;
        const lastPinned = grid.querySelector('.pinnato:last-of-type');
        if (lastPinned) lastPinned.insertAdjacentHTML('afterend', cardHTML);
        else grid.insertAdjacentHTML('afterbegin', cardHTML);
    ...
    closeLinkModal();
    try {
        await fetch(SCRIPT_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ service: "note", text: linkText }) });
...
async function toggleArchive() {
    ...
    await fetch(SCRIPT_URL, { method: 'POST', body: JSON.stringify({ service: 'update_note_type', id: currentNoteData.id, type: newType }) });
...
function showCustomAlert(message, isSuccess = false) {
    ...
    if (!bubble || !text) return;
    text.innerHTML = `
        <div style="font-size:0.7rem; color:${isSuccess ? 'var(--accent)' : '#ff4d6d'}; margin-bottom:8px; letter-spacing:1px; text-transform:uppercase;">
            ${isSuccess ? '✓ Operazione completata' : '✗ Errore sistema'}
        </div>
        <div style="color:var(--text); font-size:0.9rem; margin-top:10px;">${message}</div>
    `;
    bubble.classList.add('active');
...
async function toggleCriticAI() {
    ...
    COMPITO: Fai un'osservazione breve (max 30 parole) sui suoi gusti e suggerisci un genere/titolo che dovrebbe esplorare basato sui dati. Non usare introduzioni tipo "Ecco l'analisi".`;
    try {
        const standardResponse = await fetch(SCRIPT_URL, { method: "POST", body: JSON.stringify({ action: 'askAnalyst', prompt: prompt }) });
        const text = await standardResponse.text();
        bubble.innerHTML = `<div style="color:var(--text); font-size:0.85rem; line-height:1.5;">${text}</div>`;
    } catch (e) {
